...
development projects. Visitors browse project Boards and Pins without registration;
Pinterest OAuth, search, synchronization, and catalog management stay on a separate
...
| --- | --- | --- |
| Public | `public` or omitted | Home, Boards, individual collections, About, Contact, Privacy, Terms |
| Admin | `admin` | GitHub owner authentication, Pinterest OAuth/API, catalog CRUD, audit and export controls |
...
| Route | Description |
| --- | --- |
| `/` | Product overview and featured collections |
| `/boards` | Searchable public collection catalog |
| `/boards/[id]` | Every published Pin in one collection |
...
| `/admin` | 404 | Pinterest connection, search, catalog management, status and audit |
| `/api/pinterest/*` | 404/forbidden | Owner-authenticated read-only Pinterest API |
| `/api/projects`, `/api/collections`, `/api/references` | 404/forbidden | Owner-authenticated catalog mutations |
| `/api/admin/*` | 404/forbidden | Owner-only export and maintenance |

There is no public registration, AI runtime, Agent API, or Pinterest write scope.

## Data boundary

The public catalog includes only active projects and non-archived references. It may
display collection names and descriptions plus sanitized Pin titles, descriptions,
remote image URLs, attribution usernames, and original Pinterest source links.

It never exposes owner notes, favorites, GitHub IDs, OAuth sessions, access or refresh
tokens, secrets, rate-limit records, or audit events. Pinterest media remains remote and
is not copied into application storage.
